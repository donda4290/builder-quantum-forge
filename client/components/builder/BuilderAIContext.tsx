...
const BuilderAIContext = createContext<BuilderAIContextType | undefined>(undefined);
  ...
  const generateSuggestions = useCallback((): AISuggestion[] => {
    ...
    if (!context.currentPage || context.recentActions.length === 0) {
      ...
      contextualSuggestions.push({
        ...
        description: 'Every great website starts with an eye-catching hero section',
        type: 'component',
        priority: 'high',
    ...
    if (context.recentActions.includes('hero') && !context.recentActions.includes('cta')) {
...
export function useSmartAssistant() {
  ...
  const getCompletionTips = useCallback(() => {
    ...
    if (context.recentActions.includes('hero') && !context.recentActions.includes('features')) {
  ...
  const analyzePageStructure = useCallback(() => {
    ...
    analysis.completionScore = (presentElements.length / essentialElements.length) * 100;
