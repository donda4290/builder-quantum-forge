...
interface DomainContextType {
  ...
  createDNSRecord: (
    domainId: string,
    record: Omit<DNSRecord, "id" | "domainId" | "status">,
  ) => void;
  ...
  createSubdomain: (
    domainId: string,
    subdomain: Omit<Subdomain, "id" | "domainId" | "status">,
  ) => void;
  ...
  createRedirect: (
    domainId: string,
    redirect: Omit<DomainRedirect, "id" | "domainId" | "status">,
  ) => void;
  updateRedirect: (
    redirectId: string,
    updates: Partial<DomainRedirect>,
  ) => void;
  ...
  createEnvironment: (
    domainId: string,
    environment: Omit<DeploymentEnvironment, "id" | "domainId" | "status">,
  ) => void;
  ...
  updateProvider: (
    providerId: string,
    updates: Partial<DomainProvider>,
  ) => void;
}
...
export function DomainProvider({ children }: { children: React.ReactNode }) {
  ...
  const [domainProviders, setDomainProviders] =
    useState<DomainProvider[]>(mockProviders);
  ...
  const purchaseDomain = useCallback(
    async (domain: string, registrar: string, years: number) => {
  ...
  const updateDomain = useCallback(
    (domainId: string, updates: Partial<Domain>) => {
      setDomains((prev) =>
        prev.map((domain) =>
  ...
  const updateDNSRecord = useCallback(
    (recordId: string, updates: Partial<DNSRecord>) => {
      setDnsRecords((prev) =>
        prev.map((record) =>
  ...
  const updateSubdomain = useCallback(
    (subdomainId: string, updates: Partial<Subdomain>) => {
      setSubdomains((prev) =>
        prev.map((subdomain) =>
  ...
  const deleteSubdomain = useCallback((subdomainId: string) => {
    setSubdomains((prev) =>
      prev.filter((subdomain) => subdomain.id !== subdomainId),
    );
  }, []);
  ...
  const updateRedirect = useCallback(
    (redirectId: string, updates: Partial<DomainRedirect>) => {
      setRedirects((prev) =>
        prev.map((redirect) =>
  ...
  const deleteRedirect = useCallback((redirectId: string) => {
    setRedirects((prev) =>
      prev.filter((redirect) => redirect.id !== redirectId),
    );
  }, []);

  // SSL actions
  const setupSSL = useCallback(
    async (domainId: string, provider: string, wildcard = false) => {
  ...
  const deployWebsite = useCallback(async (environmentId: string) => {
    ...
    setDeploymentEnvironments((prev) =>
      prev.map((env) =>
        env.id === environmentId
          ? { ...env, status: "building" as const }
          : env,
      ),
    );
  ...
  const rollbackDeployment = useCallback(
    async (environmentId: string, deploymentId: string) => {
      await new Promise((resolve) => setTimeout(resolve, 5000));
  ...
  // Hosting actions
  const updateHostingSettings = useCallback(
    (domainId: string, settings: Partial<HostingSettings>) => {
      setHostingSettings((prev) => {
        const existing = prev.find((s) => s.domainId === domainId);
        if (existing) {
          return prev.map((s) =>
            s.domainId === domainId ? { ...s, ...settings } : s,
          );
        } else {
  ...
  const updateProvider = useCallback(
    (providerId: string, updates: Partial<DomainProvider>) => {
      setDomainProviders((prev) =>
        prev.map((provider) =>
