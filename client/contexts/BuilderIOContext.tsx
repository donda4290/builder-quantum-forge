...
interface BuilderIOContextType {
  ...
  // Actions
  configure: (
    config: Omit<BuilderIOConfig, "isConfigured">,
  ) => Promise<boolean>;
  createContent: (model: string, data: any) => Promise<BuilderIOContent>;
  updateContent: (
    id: string,
    updates: Partial<BuilderIOContent>,
  ) => Promise<void>;
  ...
  // Models
  createModel: (
    model: Omit<BuilderIOModel, "name"> & { name: string },
  ) => Promise<void>;
  updateModel: (
    name: string,
    updates: Partial<BuilderIOModel>,
  ) => Promise<void>;
...
const BuilderIOContext = createContext<BuilderIOContextType | undefined>(
  undefined,
);
...
export function BuilderIOProvider({ children }: { children: React.ReactNode }) {
  ...
  const [selectedContent, setSelectedContent] =
    useState<BuilderIOContent | null>(null);
  const [analytics, setAnalytics] = useState<BuilderIOAnalytics | null>(
    mockAnalytics,
  );
  ...
  const isConfigured =
    config.isConfigured && config.publicApiKey && config.privateApiKey;

  // Configuration
  const configure = useCallback(
    async (
      newConfig: Omit<BuilderIOConfig, "isConfigured">,
    ): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
  ...
  const createContent = useCallback(
    async (model: string, data: any): Promise<BuilderIOContent> => {
      setIsLoading(true);
      try {
  ...
  const updateContent = useCallback(
    async (id: string, updates: Partial<BuilderIOContent>): Promise<void> => {
      setIsLoading(true);
      try {
  ...
  const createModel = useCallback(
    async (model: BuilderIOModel): Promise<void> => {
      setIsLoading(true);
      try {
        await new Promise((resolve) => setTimeout(resolve, 1500));
  ...
  const updateModel = useCallback(
    async (name: string, updates: Partial<BuilderIOModel>): Promise<void> => {
      setIsLoading(true);
      try {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        setModels((prev) =>
  ...
  const fetchAnalytics = useCallback(
    async (dateRange?: { start: Date; end: Date }): Promise<void> => {
      setIsLoading(true);
      try {
        await new Promise((resolve) => setTimeout(resolve, 2000));
  ...
  const syncWithBuilderIO = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to sync with Builder.io",
      );
    } finally {
      setIsLoading(false);
    }
